...
async function main(): Promise<void> {
  ...
  const state = service.getState(threads);
  ...
  assert.deepEqual(service.getState(threads).unclassifiedThreads.map((thread) => thread.id), ["thread-a", "thread-b"]);
  await service.associateThread("thread-a", first);
  await service.toggle(first);
  assert.equal(service.getState(threads).directories[0]?.expanded, false);
